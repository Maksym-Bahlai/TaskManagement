...
public class UsersRepository(AppDbContext _dbContext) : IUsersRepository
{
    ...
    {
        return await _dbContext.Users.Include(user => user.Tasks).FirstOrDefaultAsync(user => user.UserName == userName, cancellationToken);
    }

    public async Task<List<User>?> GetAllAsync(CancellationToken cancellationToken)
    {
        return await _dbContext.Users.Include(user => user.Tasks).ToListAsync(cancellationToken: cancellationToken);
    }
